...
public class Tools {
	...
	public static void copy(Object source, Object destination) throws Exception{
		...
		PropertyDescriptor[] destPropertyDescriptors = destbi
		...
		for (PropertyDescriptor pd : destPropertyDescriptors) {
			...
			if (sourceip.containsKey(propertyName)) {
				...
				if (pd.getPropertyType()
						.isAssignableFrom(spd.getPropertyType())) {
					// set value

					Method wm = pd.getWriteMethod();
					if (wm == null) {
						System.out.println("no write method for property "
								+ propertyName);
						continue;
					}
					Method rm = spd.getReadMethod();
					if (rm == null) {
						System.out.println("no read method for property "
								+ propertyName);
						continue;
					}
